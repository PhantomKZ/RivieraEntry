...
async function listRequests({ q, range } = {}){
  ...
  const rows = await all(`SELECT id, createdAt, fullName, destination, fromWhom, purpose, lang FROM requests ${where} ORDER BY datetime(createdAt) ASC`, params);
  return rows;
}

module.exports = { init, createRequest, listRequests };
