    ...
    <main class="app-main app-main-wide">
      ...
      <section v-else class="card">
        ...
        <div class="actions-panel" :class="{ open: actionsOpen }" style="flex-wrap:wrap">
          ...
          <button class="btn" v-if="hasSelection" @click="deleteSelected" style="background:#b91c1c;color:#fff">Удалить</button>
        ...
        <div class="table-wrap" v-show="!(actionsOpen && isMobile)">
...
const isMobile = computed(() => window.innerWidth <= 670);
...
function doLogin(){
  fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ login: login.value, password: password.value }) })
    ...
    .then(d => { token.value = d.token; role.value = d.role || 'admin'; isAuthed.value = true; load(); })
...
function load(){
  ...
  fetch('/api/requests?' + params.toString())
    ...
    .then(d => {
      ...
      // сбрасываем выбор, если элементы отсутствуют
